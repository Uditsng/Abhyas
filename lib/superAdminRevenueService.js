...
export async function getEarningsAndCommission() {
  ...
  snapshot.forEach(doc => {
    ...
    const promotionFee = Number(data.promotionFee || 0);
    totalCommission += (baseCommission + promotionFee);
...
export async function getMonthlyRevenue() {
  ...
  snapshot.forEach(doc => {
    ...
    const baseCommission = Number(data.commission || 0);
    const promotionFee = Number(data.promotionFee || 0);
    revenue[month].commission += (baseCommission + promotionFee);
