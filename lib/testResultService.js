...
export const saveTestResult = async (userId, resultData) => {
  ...
  if (!resultData || !resultData.testId || !resultData.courseId) {
    console.error("Invalid result data provided for saving.");
    return false;
  }

  // Create a unique ID for the result document, combining testId, courseId, and a timestamp
  const resultDocId = `${resultData.testId}_${resultData.courseId}_${Date.now()}`;

  try {
    const userResultsCollectionRef = collection(db, 'users', userId, 'testResults');
    await setDoc(doc(userResultsCollectionRef, resultDocId), {
      ...resultData,
 ...
 */
export const getTestResult = async (userId, testId, courseId) => {
  if (!userId || !testId || !courseId) {
    console.error("User ID, Test ID, and Course ID are required to fetch test result.");
    return null;
  }
  ...
  try {
    const userResultsCollectionRef = collection(db, 'users', userId, 'testResults');
    const q = query(
      userResultsCollectionRef,
      where("testId", "==", testId),
      where("courseId", "==", courseId),
      orderBy("createdAt", "desc"), // Get the latest result if multiple exist
      limit(1)
    );
    ...
    if (!querySnapshot.empty) {
      ...
      console.log(`No test result found for user ${userId}, test ${testId}, course ${courseId}.`);
      return null;
    }
  } catch (error) {
