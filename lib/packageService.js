...
export const enrichPackage = async (pkg) => {
...
export const getPackagesByIds = async (packageIds = []) => {
  ...
  try {
    const packages = [];
    const chunkSize = 30;
    for (let i = 0; i < packageIds.length; i += chunkSize) {
      const chunk = packageIds.slice(i, i + chunkSize);
      const packagesQuery = query(collection(db, 'packages'), where(documentId(), 'in', chunk));
