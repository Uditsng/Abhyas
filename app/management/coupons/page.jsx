// app/management/coupons/page.jsx
...
const CouponManagementPage = () => {
    ...
    const fetchData = async () => {
        if (!user) return;
        try {
            const [couponsData, packagesData] = await Promise.all([
                getCoupons(user.uid), 
                getAllPackages()
            ]);
    ...
    const handleCreateCoupon = async (e) => {
        e.preventDefault();
        if (!newCoupon.code || !newCoupon.value || !newCoupon.expiryDate || selectedPackages.length === 0) {
            alert('Please fill all required fields, including selecting at least one package.');
            return;
        }
        try {
            await createCoupon({
        ...
        <div className="container mx-auto p-4 md:p-8 text-gray-800 dark:text-white">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md mb-8">
                ...
                <form onSubmit={handleCreateCoupon} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-2">Applicable Packages (Select at least one)</label>
                        <div className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg max-h-48 overflow-y-auto space-y-2">
                            {packages.length > 0 ? packages.map(pkg => (
                                <label key={pkg.id} className="flex items-center space-x-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                                ...
                                <p className="text-gray-500">No packages have been created yet.</p>
                            )}
                        </div>
                    </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                <h2 className="text-2xl font-semibold mb-4">Your Created Coupons</h2>
